...
var PROJECT = 'Fraction.js';

var filename = ROOT + "Original/" + PROJECT;  //process.argv[2];
var outFilename = ROOT + "Instrumented-Loop/" + PROJECT;  //process.argv[2];
...
function leave(node, parent){
    if(node.type === util.astNodes.FOR_STATEMENT) {
        var variableName;
        if (node.update != null && node.update.argument != null) {
            variableName = node.update.argument.name;
        }

        for (x = 0; x < modifiedVariables.length; x++) {
            if(modifiedVariables[x]!== undefined && modifiedVariables[x] === variableName) {
                var ii = 1;
                while(ii<=scopeChain.length) {
                    if(scopeChain[scopeChain.length - ii].indexOf(modifiedVariables[x])>-1) break;
                    ii++;
                }
                if(ii<=scopeChain.length && node.body != null && node.body.type === util.astNodes.BLOCK_STATEMENT) {
                    xx = "try { var instrument_fs = require('fs'); instrument_fs.appendFileSync('"+LOG_FILE+"', '..." + methodName[methodName.length-ii] + "-_-_-_-Changed Variable [" + modifiedVariables[x] + "] value ['+" + modifiedVariables[x] + "+']'); } catch (err){}";
                    node.body.body.unshift(esprima.parse(xx));
                    count ++;
                }
            }
            ...
            for (x = 0; x < modifiedVariables.length; x++) {
                if(modifiedVariables[x]!== undefined) {
                    ...
                    if(ii<=scopeChain.length) {
                       ...
                       /* xx = "try { if(typeof " + modifiedVariables[x] + " == 'number') { var instrument_fs = require('fs'); instrument_fs.appendFileSync('"+LOG_FILE+"', '..." + methodName[methodName.length-ii] + "-_-_-_-Changed Variable [" + modifiedVariables[x] + "] value ['+" + modifiedVariables[x] + "+']'); } } catch (err){}";
